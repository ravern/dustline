...
export type WeaponId = 'intervention' | 'ak47' | 'scar' | 'm9' | 'deagle' | 'glock' | 'knife';
export type PrimaryId = 'intervention' | 'ak47' | 'scar';
export type SecondaryId = 'm9' | 'deagle' | 'glock';
export type GrenadeKind = 'frag' | 'flash';
...
export const MODES: Record<GameMode, { name: string; maxPlayers: number; defaultLimit: number }> = {
  ffa: { name: 'Free for All', maxPlayers: 32, defaultLimit: 20 },
  tdm: { name: 'Team Deathmatch', maxPlayers: 32, defaultLimit: 50 },
  ctf: { name: 'Capture the Flag', maxPlayers: 32, defaultLimit: 3 },
};
export interface Loadout { primary: PrimaryId; secondary: SecondaryId }
export interface Vec3 { x: number; y: number; z: number }
export interface Input { seq: number; yaw: number; pitch: number; forward: number; right: number; jump: boolean; sprint: boolean; crouch: boolean; ads: boolean; fire: boolean; reload: boolean; frag?: boolean; flash?: boolean; slot: Slot; time: number; viewTime?: number; matchId?: string }
export interface Body extends Vec3 { vx: number; vy: number; vz: number; yaw: number; pitch: number; grounded: boolean; stance: 'stand' | 'crouch' | 'slide'; slideTime: number; slideCooldown: number; jumpHeld: boolean; crouchHeld: boolean; vault?: { elapsed: number; duration: number; from: Vec3; to: Vec3; height: number } }
export interface PlayerState { id: string; name: string; bot: boolean; team: Team | null; body: Body; hp: number; kills: number; deaths: number; captures: number; loadout: Loadout; slot: Slot; ammo: number[]; reserve: number[]; reloading: number; nextFire: number; ads: boolean; adsSince: number; ack: number; respawnAt: number; protectedUntil: number; lastDamage: number; grenades: Record<GrenadeKind, number>; nextGrenade: number; flashUntil: number; flashStrength: number }
...
export interface GrenadeState { id: string; kind: GrenadeKind; owner: string; team: Team | null; position: Vec3; velocity: Vec3; thrownAt: number; detonateAt: number }
export interface GameEvent { id: number; type: 'shot' | 'hit' | 'kill' | 'respawn' | 'start' | 'end' | 'flag_pickup' | 'flag_drop' | 'flag_return' | 'flag_capture' | 'grenade_throw' | 'grenade_explode' | 'flash'; player?: string; target?: string; weapon?: WeaponId; grenade?: GrenadeKind; grenadeId?: string; strength?: number; duration?: number; damage?: number; headshot?: boolean; quickscope?: boolean; from?: Vec3; to?: Vec3; team?: Team; time: number }
export interface Snapshot { type: 'snapshot'; time: number; tick: number; matchId: string; players: PlayerState[]; grenades: GrenadeState[]; events: GameEvent[]; endsAt: number; state: RoomInfo['state']; mode: GameMode; map: MapId; teamScores: Record<Team, number>; flags: FlagState[]; winner: string | null }
