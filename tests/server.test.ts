...
test('a late human replaces a bot in a full live match and inherits the requested loadout', () => {
  ...
  game.receive('a', { type: 'create', name: 'Alpha', loadout: loadout(), bots: 31 }, 1000); game.receive('a', { type: 'start' }, 1000);
  const room = game.peers.get('a')!.room!; assert.equal(room.players.size, 32);
  game.receive('b', { type: 'join', code: room.code, name: 'Bravo', loadout: loadout('scar') }, 1002);
  assert.equal(room.players.size, 32); assert.equal([...room.players.values()].filter(p => p.bot).length, 30); assert.equal(room.players.get('b')!.ammo[0], 20); assert.equal(room.players.get('b')!.loadout.primary, 'scar');
});
