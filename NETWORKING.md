...
Dustline runs an authoritative Node.js server with browser WebSocket clients. Clients send controls; the server owns collision, damage, ammunition, respawns, teams, scores, flags and match state. Each mode supports up to 32 players, with at most 16 players per team in Team Deathmatch and Capture the Flag. Humans must supply a name.
...
WebSocket provides ordered reliable TCP delivery, so packet loss can delay subsequent messages. The client checks liveness, and both ends close congested sockets rather than silently dropping movement while allowing prediction to drift. Reconnection then uses the session mechanism. Message size, message rate, command batch length, queue length and room count are bounded; malformed inputs and stale sequences are rejected. Recent gameplay events repeat in snapshots with unique IDs for client deduplication. The browser negotiates the `dustline.snapshot.v1` WebSocket subprotocol for compact positional JSON snapshots. Frames remain self-contained and preserve every numeric value and event; there are no lossy quantization or delta dependencies. Clients without that subprotocol receive the original object JSON format. Each snapshot is serialized once per format for all recipients. The mixed-client 32-player tests measured 54–55% smaller snapshot payloads at the same 20 Hz, without per-message compression.
...
With a server running, `node --import tsx tests/multiplayer.mjs` exercises real WebSocket rooms with 32 players in each of FFA, TDM and CTF. It checks room limits, map and team replication, 150 acknowledged inputs per player, a 300 ms stalled sender, simulated transit delays, identity-preserving reconnect and token privacy. `DUSTLINE_TEST_URL` optionally selects a test instance. It prints measured snapshot rate and mean payload size. Set `DUSTLINE_TEST_WIRE=compact` to compare compact and legacy snapshots from the same tick, including gameplay events and resumed sessions.
